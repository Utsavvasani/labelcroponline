...
export function MeeshoToolClient() {
  ...
  const getActiveModeName = () => {
    if (cropMode === "custom") return "Custom";
    if (cropMode === "invoice") return "Standard Cut";
    if (cropMode === "label_sku") return "Half Cut";
    return "Standard Cut";
  };
              ...
              <div className="relative z-30 px-3 sm:px-4 py-2 bg-white border-b border-slate-300 rounded-t-md flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <div className="flex items-center justify-between sm:justify-start gap-2.5 sm:gap-3 w-full sm:w-auto">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                    ...
                    <span className="font-semibold text-xs sm:text-sm text-black truncate">
                      Meesho Cropper
                    </span>
                  </div>

                  {/* Right side on mobile: All quick action icons (Compact Toolstrip) */}
                  <div className="flex sm:hidden items-center gap-1.5 shrink-0">
                    <TooltipButton
                      icon={Plus}
                      label="Add and merge another PDF"
                    ...
                    <TooltipButton
                      icon={RotateCcw}
                      label="Clear and start new crop"
                      onClick={handleReset}
                      alignRight={true}
                    />
                  </div>
                ...
                {/* Crop Mode Switcher Pills (Centered) */}
                <div className="flex items-center justify-center w-full sm:w-auto my-0.5 sm:my-0">
                  <div className="flex bg-slate-100 p-0.5 rounded border border-slate-300 h-[34px] w-full sm:w-auto">
                    <button
                      type="button"
                      onClick={() => handleModeChange("invoice")}
                      ...
                      title="Standard Cut: Full label with Tax Invoice"
                    >
                      Standard Cut
                    </button>
                    <button
                      ...
                      title="Half Cut: Label + SKU Details (Compact thermal sticker)"
                    >
                      Half Cut
                    </button>
                    <button
                ...
                {/* Actions on Desktop (hidden on mobile since shown in Row 1) */}
